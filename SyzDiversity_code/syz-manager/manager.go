...
type Manager struct {
	...
	ClusterInfo     *corpus.ClusterInfo // Added field
...
// LoadClusterInfo loads cluster information from CSV file
func LoadClusterInfo(path string) (*corpus.ClusterInfo, error) {
	info := &corpus.ClusterInfo{
	...
	for {
		...
		info.ProgramHashes[hash] = clusterID
		info.ClusterSizes[clusterID]++
	}
...
func (mgr *Manager) loadCorpus(enabledSyscalls map[*prog.Syscall]bool) []fuzzer.Candidate {
	...
	mgr.ClusterInfo = corpus.NewClusterInfo()
	clusterFilePath := filepath.Join(mgr.cfg.Workdir, "cluster_info.csv")
	if err := mgr.ClusterInfo.LoadFromCSV(clusterFilePath); err != nil {
		log.Logf(0, "failed to load cluster info from %s: %v", clusterFilePath, err)

		// If there is no cluster info file, perform dynamic cluster assignment for all seeds
		log.Logf(0, "performing dynamic cluster assignment for all seeds")
		for _, candidate := range ret.Candidates {
			if candidate.Prog != nil {
				// Use GetProgHash function to get hash
				progHash := GetProgHash(candidate.Prog)
				
				clusterID, isNewCluster := mgr.ClusterInfo.AssignCluster(candidate.Prog, progHash)
				
				if isNewCluster {
					log.Logf(1, "Created new cluster %d for corpus seed %s", clusterID, progHash[:8])
				}
				
				// Store program in cluster
				mgr.ClusterInfo.StoreProgramInCluster(candidate.Prog, progHash, clusterID)
			}
		}
		} else {
			log.Logf(0, "loaded cluster info with %d clusters", len(mgr.ClusterInfo.ClusterSizes))

			// Check if candidate seeds all have cluster IDs, perform dynamic assignment for those that don't
			for _, candidate := range ret.Candidates {
				if candidate.Prog != nil {
					progHash := GetProgHash(candidate.Prog)
					if _, exists := mgr.ClusterInfo.GetClusterID(progHash); !exists {

						clusterID, isNewCluster := mgr.ClusterInfo.AssignCluster(candidate.Prog, progHash)
						
						if isNewCluster {
							log.Logf(1, "Created new cluster %d for existing corpus seed %s", clusterID, progHash[:8])
						} else {
							log.Logf(3, "Assigned corpus seed %s to existing cluster %d", progHash[:8], clusterID)
						}
	...
	// Reserved cluster center seed loading for future extension
	clusterCorePath := filepath.Join(mgr.cfg.Workdir, "cluster_core.csv")
	if err := mgr.ClusterInfo.LoadClusterCores(clusterCorePath); err != nil {
		log.Logf(1, "failed to load cluster cores: %v", err)
	}

	// Use two-level sorting: first sort by cluster density, then by call count
	sort.SliceStable(ret.Candidates, func(i, j int) bool {
		hashI := GetProgHash(ret.Candidates[i].Prog)
		hashJ := GetProgHash(ret.Candidates[j].Prog)

		clusterI, _ := mgr.ClusterInfo.GetClusterID(hashI)
		clusterJ, _ := mgr.ClusterInfo.GetClusterID(hashJ)

		// Get cluster size
		sizeI := mgr.ClusterInfo.GetClusterSize(clusterI)
		sizeJ := mgr.ClusterInfo.GetClusterSize(clusterJ)

		// If cluster sizes are different, prioritize smaller clusters
		if sizeI != sizeJ {
			return sizeI < sizeJ
		}

		// If cluster sizes are the same, sort by call count
...
func (mgr *Manager) MachineChecked(features flatrpc.Feature, enabledSyscalls map[*prog.Syscall]bool) queue.Source {
	...
	if mgr.mode == ModeFuzzing || mgr.mode == ModeCorpusTriage {
		...
		fuzzerObj := fuzzer.NewFuzzer(context.Background(), &fuzzer.Config{
			...
			ClusterInfo:    mgr.ClusterInfo, // Pass cluster information
