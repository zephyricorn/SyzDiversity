...
type Request struct {
	...
	// 新增字段：支持原始种子追踪
	OriginSeedHash   string     // 原始种子的hash（用于突变种子追踪其来源）
	MutationDepth    int        // 突变深度（0=生成，1=第一代突变，2=第二代突变...）
	IsGenerated      bool       // 是否为全新生成的程序（非突变）
...
func Plain() *PlainQueue {
	return &PlainQueue{
		queue: make([]*Request, 0), // Explicitly initialize as empty slice instead of nil
	}
}
...
// SortItems sorts items in the queue according to the given comparison function
func (q *PlainQueue) SortItems(less func(a, b *Request) bool) {
	q.mu.Lock()
	defer q.mu.Unlock()

	sort.Slice(q.queue, func(i, j int) bool {
		return less(q.queue[i], q.queue[j])
	})
}

// GetAllRequests returns a copy of all requests in the queue
func (q *PlainQueue) GetAllRequests() []*Request {
	if q == nil {
		return nil
	}
	...
	if q.queue == nil {
		return []*Request{} // Return empty slice instead of nil
	}

	result := make([]*Request, len(q.queue))
	copy(result, q.queue)
	return result
}

// ReplaceRequests replaces queue content with new request list
func (q *PlainQueue) ReplaceRequests(newRequests []*Request) {
	q.mu.Lock()
	defer q.mu.Unlock()

	q.queue = make([]*Request, len(newRequests))
	copy(q.queue, newRequests)
	q.pos = 0
}
