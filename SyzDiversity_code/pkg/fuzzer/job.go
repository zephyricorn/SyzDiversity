...
func genProgRequest(fuzzer *Fuzzer, rnd *rand.Rand) *queue.Request {
	...
	return &queue.Request{
		...
		IsGenerated:  true, // 标记为全新生成
		MutationDepth: 0,   // 生成的程序深度为0
	}
}
...
func mutateProgRequest(fuzzer *Fuzzer, rnd *rand.Rand) *queue.Request {
	...
	// Use UCB algorithm to select program
	if fuzzer.ClusterInfo != nil && fuzzer.Config.Corpus != nil {
		p = fuzzer.Config.Corpus.ChooseProgramCommunityUCB(rnd, fuzzer.ClusterInfo)
	} else {
	...
	return &queue.Request{
		...
		OriginSeedHash: originHash,    // 记录原始种子hash
		MutationDepth:  1,             // 第一代突变
		IsGenerated:    false,         // 标记为突变产生
	}
}
...
func (job *triageJob) handleCall(call int, info *triageCall) {
	...
	// Assign cluster ID for new seed (before saving to Corpus)
	if job.fuzzer.ClusterInfo != nil {
		hash := GetProgHash(p)

		// Check if cluster ID already exists
		if existingClusterID, exists := job.fuzzer.ClusterInfo.GetClusterID(hash); exists {
			// Seed already has cluster assignment, log and use existing assignment
			job.fuzzer.Logf(3, "Using existing cluster %d for new corpus input %s",
				existingClusterID, hash)

			// If seed is cluster center, store its program object
			job.fuzzer.ClusterInfo.StoreProgramInCluster(p, hash, existingClusterID)
		} else {
			// Seed has no cluster assignment, perform dynamic classification
			clusterID, isNewCluster := job.fuzzer.ClusterInfo.AssignCluster(p, hash)

			if isNewCluster {
				job.fuzzer.Logf(0, "Created new cluster %d for new corpus input %s", clusterID, hash)
			} else {
				//job.fuzzer.Logf(0, "Assigned new corpus input %s to cluster %d", hash, clusterID)
			}

			// If seed is cluster center, store its program object
			job.fuzzer.ClusterInfo.StoreProgramInCluster(p, hash, clusterID)
		}
	}
...
func (job *triageJob) deflake(exec func(*queue.Request, ProgFlags) *queue.Result) (stop bool) {
	...
	for run := 1; ; run++ {
		...
		result := exec(&queue.Request{
			...
			IsGenerated:     false,  // triage中的程序通常来自候选或corpus
			MutationDepth:   0,      // triage不是突变，深度为0
		}, progInTriage)
		if result.Stop() {
			return true
		}
	...
	p, call := prog.Minimize(job.p, call, mode, func(p1 *prog.Prog, call1 int) bool {
		...
		for i := 0; i < minimizeAttempts; i++ {
			result := job.execute(&queue.Request{
				...
				IsGenerated:     false,  // minimize不是生成操作
				MutationDepth:   0,      // minimize不是突变，深度为0
			}, 0)
			if result.Stop() {
				stop = true
				return false
